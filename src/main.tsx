...
function WaveformTrim({
  ...
  useEffect(() => {
    ...
    for (let x = 0; x < width; x += 1) {
      ...
      for (let index = 0; index < stride; index += 1)
        peak = Math.max(peak, Math.abs(samples[x * stride + index] ?? 0));
  ...
  function beginDrag(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    const edge =
      explicitEdge ??
      (Math.abs(pointerPercent - startPercent) < Math.abs(pointerPercent - endPercent)
        ? 'start'
        : 'end');
    event.currentTarget.setPointerCapture(event.pointerId);
    setDragging(edge);
    moveHandle(event, edge);
  }
  return (
    <div
      ref={rootRef}
      className={`waveform-trimmer ${dragging ? 'dragging' : ''}`}
      onPointerDown={beginDrag}
      onPointerMove={(event) => {
        if (dragging) moveHandle(event, dragging);
      }}
    ...
    >
      <canvas
        ref={canvasRef}
        aria-label='Drag the waveform handles to choose the audio trim range'
      />
      <div
      ...
      <div
        ...
        aria-label={`Trim start at ${formatTime(range.start)}`}
      />
      <div
        className='trim-handle end'
        data-edge='end'
        style={{ left: `${endPercent}%` }}
        aria-label={`Trim end at ${formatTime(range.end)}`}
      />
    </div>
  );
}
...
function WaveformPlayback({ source, duration }: { source: string; duration: number }) {
  ...
  return (
    <div className='waveform-playback'>
      <audio
        ref={audioRef}
        src={source}
        preload='metadata'
...
function App() {
  ...
  async function refreshRecordingState() {
    try {
      setRecording(await popupCommand<RecordingState>('POPUP_GET_STATE'));
    } catch {
  ...
  useEffect(() => {
    if (recording.status !== 'ready' || loadedRevisionRef.current === recording.revision)
      return;
    ...
    void (async () => {
      try {
        ...
        setScreen('editor');
        setMessage('Recording restored. Drag the waveform handles to set the trim range.');
      } catch (caught) {
        setError(
          caught instanceof Error ? caught.message : 'Could not restore the recording.',
        );
      }
    })();
  ...
  async function controlRecorder(
    type: 'POPUP_START' | 'POPUP_PAUSE' | 'POPUP_RESUME' | 'POPUP_FINISH' | 'POPUP_CANCEL',
  ) {
    setError(null);
    try {
      if (type === 'POPUP_START') {
  ...
  async function exportMp3() {
    ...
    try {
      ...
      link.href = href;
      link.download = `tab-audio-${new Date().toISOString().replace(/[:.]/g, '-')}.mp3`;
      link.click();
      window.setTimeout(() => URL.revokeObjectURL(href), 1_000);
      const cleared = await popupCommand<RecordingState>('POPUP_DISCARD_SAVED');
      setRecording(cleared);
      setSource(null);
      setEdited(null);
      ...
      setError(caught instanceof Error ? caught.message : 'MP3 export failed.');
    } finally {
      setExporting(false);
    }
  }

  const isCapturing = ['starting', 'recording', 'paused', 'processing'].includes(
    recording.status,
  );
  ...
  return (
    <main className='app'>
      ...
      <nav aria-label='Extension screens'>
        <button
          className={screen === 'record' ? 'active' : ''}
          onClick={() => setScreen('record')}
        >
          Record
        </button>
        <button
          className={screen === 'editor' ? 'active' : ''}
            ...
            <>
              {recording.status === 'processing' ? (
                <div className='capture-card'>
                  <p>Finishing your recording</p>
                  <strong>…</strong>
                  <span>Saving it safely to this extension</span>
                </div>
              ) : (
                <>
                  <div className='capture-card'>
                    <p>Current tab is being captured</p>
                    <strong>{formatTime(recording.elapsedMs / 1000)}</strong>
                    <span>
                      {recording.status === 'paused'
                        ? 'Recording is paused'
                        : 'Audio only · current tab'}
                    </span>
                  </div>
                  <button
                    className='pause-button'
                    onClick={() =>
                      void controlRecorder(
                        recording.status === 'paused' ? 'POPUP_RESUME' : 'POPUP_PAUSE',
                      )
                    }
                  >
                    {recording.status === 'paused'
                  ...
                  <div className='capture-actions'>
                    <button
                      ...
                      onClick={() => void controlRecorder('POPUP_FINISH')}
                    >
                      ✓ Finish recording
                    </button>
                    <button
      ...
      {screen === 'settings' && (
        <section className='panel settings'>
          <label>
            ...
            <select
              ...
              onChange={(event) =>
                setSettings((current) => ({
                  ...current,
                  bitrate: Number(event.target.value) as Settings['bitrate'],
                }))
              }
            >
          ...
          <label>
            Silence threshold <output>{settings.silenceDb} dB</output>
            <input
              type='range'
              min='-60'
              max='-20'
              ...
              onChange={(event) =>
                setSettings((current) => ({
                  ...current,
                  silenceDb: Number(event.target.value),
                }))
              }
            />
          </label>
          <label>
            Minimum silent duration <output>{settings.minimumSilenceMs} ms</output>
            <input
              type='range'
              min='150'
              max='1500'
              step='50'
              value={settings.minimumSilenceMs}
              onChange={(event) =>
                setSettings((current) => ({
                  ...current,
                  minimumSilenceMs: Number(event.target.value),
                }))
              }
            />
          </label>
          <p className='hint'>Settings are saved locally in the extension.</p>
        </section>
      )}
    </main>
  );
